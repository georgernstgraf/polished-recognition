...
**Active: #44 IME voice bar — committed + pushed (`4bdedbc`), awaiting on-device UX verify. #46 keyboard selectability — committed (`f4ebca4`), verified on-device, CLOSED.**

## What's done

### #44 — IME voice bar redesign (commit `4bdedbc`, OPEN — pending on-device UX verify)
...
## Open tasks
1. [ ] **On-device verify #44 UX** (requires the phone): `adb install -r` → switch active keyboard to Polished → pause→resume, send (commitText), cancel, gear-implicit-pause→Settings→return→Resume, spinner readability, 2nd dictation (no deadlock).
...
Last cleared: 2026-08-23. Knowledge files current (ARCHITECTURE/PITFALLS/STATE/HANDOFF updated with all session findings).
