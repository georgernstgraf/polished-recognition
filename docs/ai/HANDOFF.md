...
**2026-09-13: #77 SHIPPED pending device feel-check — pause during the pulse's deep dwell froze the IME at alpha 0.15; new `PulseAlphaPolicy` pins rows to `1f` outside RECORDING (pulse only while RECORDING). Committed `6546990`, tests 184/184 + `assembleRelease` green. Owner must `installRelease` on the device machine and confirm. v1.2.2 RELEASED 2026-09-11 (standalone issue #76): tag pushed, Play alpha VERIFIED LIVE via Play API (1.2.2 / 10202, status=completed, whatsnew attached; production/beta empty), GitHub release has AAB + reproducible APK, workflows green. #76 stays OPEN until F-Droid serves 10202 — fdroidbot auto-update pending (master fdroiddata still shows 1.2.1/10201; AutoUpdateMode: Version, no manual MR needed). #74 body: listing package renamed to v1.2.3 (F-Droid reads fastlane from the tag; 1.2.2 ships without listing assets). Groq retired `llama-3.3-70b-versatile` — local `.env` now uses `openai/gpt-oss-120b` (gitignored, CI unaffected). Next: #77 device check → close, #76 close-watch, then #74 Phase 0 (v1.2.3 listing assets), #75 rate-limit logging.**

## Open tasks

1. [ ] **#77 — IME full contrast when not recording** (open until owner on-device feel-check): code shipped/pushed (`6546990`), `PulseAlphaPolicy` + `PulseAlphaPolicyTest` + `applyAlpha()` delegation. Owner: `./gradlew installRelease` on the device machine, press pause during the visible deep phase → bar must be fully opaque; resume → pulse returns. Then close #77.
