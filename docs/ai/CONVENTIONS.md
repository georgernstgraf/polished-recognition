...
- GitHub binary assets are pruned to the newest **7 per kind** by `scripts/cleanup-github-assets.sh`, called from `build.yml` (every `master` push), `release.yml` and `fdroid-apk.yml` (both need `actions: write` + `contents: write`): Actions artifacts keep newest 7 **per name** (`app-release`, `release-aab`, `github-pages`); `build-*` releases keep 7 (page **and** git tag); orphan `build-*` tags keep 7; `v*` release pages keep 7. **`v*` git tags are never deleted** (F-Droid `UpdateCheckMode: Tags ^v` + reproducible builds). `build.yml` sets `make_latest: false` so `v*` releases stay the GitHub "Latest".
