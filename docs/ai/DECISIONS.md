...
- **Flash feedback**: while RECORDING, a repeating `ValueAnimator` on the root view's **alpha** (1.0 ↔ 0.7, 500ms/cycle, `REVERSE`, `INFINITE`) pulses the entire bar between full contrast and slight gray; cancelled in every other state (root alpha reset to 1f).
