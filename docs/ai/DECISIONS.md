...
## 2026-08-31 (round 2): Voice-reactive RMS-driven pulse (#57)
- **Choice**: The fixed-cycle flash is replaced by a microphone-loudness-driven pulse. `Event.RmsChanged` (raw 16-bit RMS per ~40–80 ms buffer, emitted continuously while RECORDING, previously ignored) feeds `RmsAlphaMapper` (pure object, unit-tested): EMA smoothing (weight 0.3), log-scaled normalization against `RMS_CEILING = 2500`, mapped to root alpha `0.5 + 0.5·level` (floor 0.5 = deep gray on silence, 1.0 = full contrast on loud speech). The IME chases the target with `View.animate()`: **slow dive 1000 ms** (target < current), **quick rise 150 ms** (target ≥ current), deadband 0.03 to skip no-op updates; `View.animate()` reuses the per-view animator and starts from the current value, so frequent RMS events chase smoothly without flicker. Reset (cancel + alpha 1f + smoother reset) on every non-RECORDING state.
- **Reason**: User feedback: the fixed pulse was "too frantic" and felt like superficial tinkering; wanted solid visual feedback. Voice-reactive gives cause-and-effect: the bar brightens with your voice and slowly sinks into gray during silence. User-selected parameters: floor 0.5, slow-dive/quick-rise easing.
- **Considered**: Just retuning the fixed cycle (2 s, deeper floor) — rejected, user chose RMS-driven; dimming only a background scrim (rejected — user wants the entire IME to pulse); linear RMS mapping (rejected — log scale feels perceptually smoother, raw RMS is heavily skewed).
- **Tradeoff**: Tuning knobs are empirical (`RMS_CEILING = 2500`, EMA 0.3, deadband 0.03) — if reaction feels weak/strong on device, `RMS_CEILING` is the primary knob. Alpha dims the icons too at the floor (accepted — "entire IME" was the explicit requirement). RmsChanged events are gated to RECORDING in `handleEvent`, so PAUSED/PROCESSING can't move the bar.
