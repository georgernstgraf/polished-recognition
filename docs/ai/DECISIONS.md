...
- **Tradeoff**: Padding operates on 1-char windows only (a multi-space run is fine — first char decides); NBSP needed an explicit union with `Character.isSpaceChar` because `isWhitespace()` misses it (see PITFALLS). Feel-check on device pending — #66 stays open until verified.
