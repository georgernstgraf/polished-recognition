...
- Translation mode injects an additional instruction into the **system** prompt via `{{translate_prompt}}`.
- Source language detection comes from Whisper's response `language` field (ISO 639-1 code), mapped to human-readable via LanguageMapper. When the field is null/blank (or literally `"unknown"`), the `{{source_language}}` sentence is dropped entirely from the system prompt.
