...
- Translation mode injects an additional instruction into the **system** prompt via `{{optional_target_language_wish}}`.
- Source language detection comes from Whisper's response `language` field (ISO 639-1 code), mapped to human-readable via LanguageMapper. When the field is null/blank (or literally `"unknown"`), the `{{optional_source_language_info}}` sentence is dropped entirely from the system prompt.
